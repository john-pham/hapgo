class Morris.Grid extends Morris.EventEmitter
  ...
  setData: (data, redraw = true) ->
    ...
    @couriers = []
    if @options.couriers.length > 0
      if @options.parseTime
        @couriers = (Morris.parseDate(e) for e in @options.couriers)
      else
        @couriers = @options.couriers
      @xmax = Math.max(@xmax, Math.max(@couriers...))
      @xmin = Math.min(@xmin, Math.min(@couriers...))
  ...
  # draw couriers vertical lines
  drawEvents: ->
    for event, i in @couriers
